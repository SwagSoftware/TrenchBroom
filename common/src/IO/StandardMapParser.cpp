...
StandardMapParser::StandardMapParser(
  std::string_view str, const Model::MapFormat sourceMapFormat,
...
void StandardMapParser::parseBrushOrBrushPrimitiveOrPatch(ParserStatus& status) {
  ...
  auto token = expect(
    QuakeMapToken::OBrace | QuakeMapToken::CBrace | QuakeMapToken::Eof, m_tokenizer.nextToken());
  ...
  token = m_tokenizer.peekToken();
  if (
    m_sourceMapFormat == Model::MapFormat::Quake3 || m_sourceMapFormat == Model::MapFormat::Doom3) {
...
void StandardMapParser::parseDoom3PrimitiveFace(ParserStatus& status) {
  ...
  const auto planeEq =
    correct(parseFloatVector<4>(QuakeMapToken::OParenthesis, QuakeMapToken::CParenthesis));
  ...
  if (!bpDegenerate(bpTexMatX, bpTexMatY)) {
    ...
    // compute a fake shift scale rot representation from the texture matrix these shift scale rot
    // values are to be understood in the local axis base
    ...
    vm::vec2f scale;
    // scale[0] = static_cast<float>( 1.0 / sqrt( bpTexMatX[0] * bpTexMatX[0] + bpTexMatY[0] *
    // bpTexMatY[0] ) ); scale[1] = static_cast<float>( 1.0 / sqrt( bpTexMatX[1] * bpTexMatX[1] +
    // bpTexMatY[1] * bpTexMatY[1] ) );
...
void StandardMapParser::parseQuake3Patch(ParserStatus& status, const size_t startLine) {
  ...
  */
  ...
  onPatch(
    startLine, lineCount, m_targetMapFormat, rowCount, columnCount, std::move(controlPoints),
    std::move(textureName), status);
}
...
void StandardMapParser::parseDoom3Patch2(ParserStatus& status, const size_t startLine) {
  ...
  // idMapPatch* idMapPatch::Parse( idLexer& src, const idVec3& origin, bool patchDef3, float
  // version )
  ...
  onPatch(
    startLine, lineCount, m_targetMapFormat, w, h, std::move(controlPoints), std::move(textureName),
    status);
}
