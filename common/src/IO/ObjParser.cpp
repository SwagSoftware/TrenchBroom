...
std::unique_ptr<Assets::EntityModel> ObjParser::doInitializeModel(Logger& logger) {
  ...
  // As such this implicitly covers situations where the default skin is intended to be used, but is
  // manually specified incorrectly.
  ...
  for (const std::string& line : kdl::str_split(m_text, "\n")) {
    ...
    if (tokens.size() != 0) {
      if (tokens[0] == "v") {
        ...
        positions.push_back(
          vm::vec3f(std::stof(tokens[1]), std::stof(tokens[2]), std::stof(tokens[3])));
      } else if (tokens[0] == "vt") {
        if (tokens.size() < 3) {
        ...
        if (tokens.size() < 2) {
          // Assume they meant "use default material" (just in case; this doesn't really make sense,
          // but...)
          current_material = 0;
        } else {
...
bool NvObjParser::transformObjCoordinateSet(
  ...
  for (vm::vec3f& pos : positions) {
    ...
    // The transform used in make_body is MAP-To-OBJ, as Neverball uses the OBJ coordinate space
    // natively. The output is (X, Z, -Y); thus the inverse transform is (X, -Z, Y)
    pos[0] *= 64.0f;
    float y = pos[1];
    pos[1] = pos[2] * -64.0f;
    pos[2] = y * 64.0f;
...
std::optional<Assets::Texture> NvObjParser::loadMaterial(const std::string& text, Logger& logger) {
  // NOTE: A reasonable solution here would be to use the same material handling as the brushes
  // unless otherwise required. Then Neverball just gets an additional texture search directory. But
  // there's raw pointers all over the Texture system, so without further details on how memory is
  // managed there, that's a bad idea.
...
std::optional<Assets::Texture> NvObjParser::loadFallbackMaterial(Logger& logger) {
  ...
  // This isn't really how it works, but the Neverball-side truth involves MAP files acting as a
  // replacement for something like JSON. This is a less Neverball-specific set of logic which
  // should be useful for any game.
...
std::optional<Assets::Texture> Doom3ObjParser::loadMaterial(
  const std::string& text, Logger& logger) {
