...
namespace TrenchBroom {
    namespace IO {
        SpriteParser::SpriteParser(std::string name, std::shared_ptr<File> file, const FileSystem& fs) :
        m_name{std::move(name)},
        m_file{std::move(file)},
        m_fs{fs} {}

        std::unique_ptr<Assets::EntityModel> SpriteParser::doInitializeModel(Logger& logger) {
        ...
        void SpriteParser::doLoadFrame(const size_t frameIndex, Assets::EntityModel& model, Logger&) {
