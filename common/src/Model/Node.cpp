...
std::vector<std::unique_ptr<Node>> Node::replaceChildren(
  ...
  auto oldChildren = kdl::vec_transform(m_children, [](Node* child) {
  ...
  addChildren(kdl::vec_transform(std::move(newChildren), [](std::unique_ptr<Node>&& child) {
...
void Node::findEntityNodesWithNumberedProperty(
  const std::string& prefix, const std::string& value, std::vector<EntityNodeBase*>& result) const {
