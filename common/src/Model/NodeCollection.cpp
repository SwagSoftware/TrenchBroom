...
bool NodeCollection::hasBrushesRecursively() const {
  ...
  for (const auto* node : m_nodes) {
    const auto hasBrush = node->accept(kdl::overload(
      [&](auto&& thisLambda, const WorldNode* world) -> bool {
        return visitChildrenAndExitEarly(thisLambda, world);
      },
      [&](auto&& thisLambda, const LayerNode* layer) -> bool {
        return visitChildrenAndExitEarly(thisLambda, layer);
      },
      [&](auto&& thisLambda, const GroupNode* group) -> bool {
        return visitChildrenAndExitEarly(thisLambda, group);
      },
      [&](auto&& thisLambda, const EntityNode* entity) -> bool {
        return visitChildrenAndExitEarly(thisLambda, entity);
      },
      [](const BrushNode*) -> bool {
        return true;
      },
      [](const PatchNode*) -> bool {
        return false;
      }));
    if (hasBrush) {
      return true;
