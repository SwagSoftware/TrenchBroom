...
bool BrushNode::contains(const Node* node) const {
  return node->accept(kdl::overload(
    ...
    [&](const GroupNode* group) {
      return m_brush.contains(group->logicalBounds());
    },
    [&](const EntityNode* entity) {
      return m_brush.contains(entity->logicalBounds());
    },
    [&](const BrushNode* brush) {
      return m_brush.contains(brush->brush());
    },
...
static bool intersectsPatch(const Brush& brush, const PatchGrid& grid) {
  ...
  // if brush contains any grid point, they intersect (or grid is contained, which we count as
  // intersection)
  for (const auto& point : grid.points) {
    if (brush.containsPoint(point.position)) {
      return true;
...
bool BrushNode::intersects(const Node* node) const {
  return node->accept(kdl::overload(
    ...
    [&](const GroupNode* group) {
      return m_brush.intersects(group->logicalBounds());
    },
    [&](const EntityNode* entity) {
      return m_brush.intersects(entity->logicalBounds());
    },
    [&](const BrushNode* brush) {
      return m_brush.intersects(brush->brush());
    },
    [&](const PatchNode* patch) {
...
bool BrushNode::allFacesHaveAnyTagInMask(TagType::Type tagMask) const {
  // Possible optimization: Store the shared face tag mask in the brush and updated it when a face
  // changes.
