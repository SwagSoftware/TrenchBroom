 ...
 *
 * The normals are computed by averaging the normals of the quadrants incident to each point.
 *
 *      *
 *    A | B
 *  *---*---* row
 ...
 *     col
 *
 * For a point at row, col, we compute the normals of the incident quadrants A, B, C, D, take their
 * average, and normalize the result. Not every grid point has four incident quadrants (e.g. the
 * corner points have only one). If the grid points of two opposing sides of the grid coincide, we
 * treat them as one grid point and average their normals.
 */
std::vector<vm::vec3> computeGridNormals(
   ...
   *     col
   *
   * Quadrant A is above and left of the grid point at row, col, quadrant B is above and right of
   * the grid point, and so on. We determine the incident grid points necessary to compute the
   * normals (via cross product). The returned normal is not normalized.
   */
  ...
  for (size_t col = 1u; col < r; ++col) {
    normals[index(t, col)] = (normalForQuadrant(t, col, RowOffset::Below, ColOffset::Left) +
                              normalForQuadrant(t, col, RowOffset::Below, ColOffset::Right)) /
                             static_cast<FloatType>(2);
    normals[index(b, col)] = (normalForQuadrant(b, col, RowOffset::Above, ColOffset::Left) +
  ...
  for (size_t row = 1u; row < b; ++row) {
    normals[index(row, l)] = (normalForQuadrant(row, l, RowOffset::Above, ColOffset::Right) +
                              normalForQuadrant(row, l, RowOffset::Below, ColOffset::Right)) /
                             static_cast<FloatType>(2);
    normals[index(row, r)] = (normalForQuadrant(row, r, RowOffset::Above, ColOffset::Left) +
  ...
  for (size_t row = 1u; row < b; ++row) {
    for (size_t col = 1u; col < r; ++col) {
      normals[index(row, col)] = (normalForQuadrant(row, col, RowOffset::Above, ColOffset::Left) +
  ...
  // Two grid points on opposing sides of the grid coincide if their distance is less than this.
  // This is from Q3 Radiant's source code.
  constexpr auto GridPointEpsilon = static_cast<FloatType>(1);

  // check opposing sides of the grid, if their corresponding points coincide, combine the normals
...
const EntityNodeBase* PatchNode::entity() const {
  return visitParent(kdl::overload(
                       [](const WorldNode* world) -> const EntityNodeBase* {
                         return world;
                       },
                       [](const EntityNode* entity) -> const EntityNodeBase* {
                         return entity;
                       },
