...
void GameFactory::initializeFileSystem() {
  ...
  const std::vector<IO::Path> gameConfigSearchDirs =
...
void GameFactory::doLoadGameConfig(const IO::Path& path) {
  ...
  m_gamePaths.insert(
    std::make_pair(configName, Preference<IO::Path>(gamePathPrefPath, IO::Path())));

  const auto defaultEnginePrefPath =
    IO::Path("Games") + IO::Path(configName) + IO::Path("Default Engine");
  m_defaultEngines.insert(
    std::make_pair(configName, Preference<IO::Path>(defaultEnginePrefPath, IO::Path())));
}
...
void GameFactory::loadCompilationConfig(GameConfig& gameConfig) {
  ...
  try {
    ...
    std::cerr << "Could not load compilation configuration '" + path.asString() +
                   "': " + std::string(e.what())
              << "\n";
...
void GameFactory::loadGameEngineConfig(GameConfig& gameConfig) {
  ...
  try {
    ...
    std::cerr << "Could not load game engine configuration '" + path.asString() +
                   "': " + std::string(e.what())
              << "\n";
...
void GameFactory::writeCompilationConfig(
  ...
  if (gameConfig.compilationConfigParseFailed) {
    ...
    logger.warn() << "Backed up malformed compilation config "
                  << m_configFS->makeAbsolute(profilesPath).asString() << " to "
                  << m_configFS->makeAbsolute(backupPath).asString();
...
void GameFactory::writeGameEngineConfig(
  ...
  m_configFS->createFileAtomic(profilesPath, stream.str());
  gameConfig.gameEngineConfig = std::move(gameEngineConfig);
  std::cout << "Wrote game engine config to " << m_configFS->makeAbsolute(profilesPath).asString()
            << std::endl;
}
} // namespace Model
} // namespace TrenchBroom
