...
void MapDocumentCommandFacade::performDeselect(const std::vector<Model::BrushFaceHandle>& faces) {
  const auto implicitlyLockedGroups = kdl::vector_set<Model::GroupNode*>{
    kdl::vec_filter(Model::findAllLinkedGroups(*m_world.get()), [](const auto* group) {
      return group->lockedByOtherSelection();
    })};
  ...
  // The strategy is to figure out what needs to be locked given m_selectedBrushFaces, and then
  // un-implicitly-lock all other linked groups.
...
MapDocumentCommandFacade::performReplaceChildren(
  ...
  for (auto& [parent, newChildren] : nodes) {
    allNewChildren =
      kdl::vec_concat(std::move(allNewChildren), kdl::vec_transform(newChildren, [](auto& child) {
                        return child.get();
                      }));
...
void MapDocumentCommandFacade::performSwapNodeContents(
  ...
  const auto nodes = kdl::vec_transform(nodesToSwap, [](const auto& pair) {
    return pair.first;
  });
  ...
  const auto [notifyTextureCollectionChange, notifyEntityDefinitionsChange, notifyModsChange] =
    notifySpecialWorldProperties(*game(), nodesToSwap);
  NotifyBeforeAndAfter notifyTextureCollections(
    notifyTextureCollectionChange, textureCollectionsWillChangeNotifier,
    textureCollectionsDidChangeNotifier);
  NotifyBeforeAndAfter notifyEntityDefinitions(
    notifyEntityDefinitionsChange, entityDefinitionsWillChangeNotifier,
...
std::map<Model::Node*, Model::VisibilityState> MapDocumentCommandFacade::setVisibilityState(
...
std::map<Model::Node*, Model::VisibilityState> MapDocumentCommandFacade::setVisibilityEnsured(
