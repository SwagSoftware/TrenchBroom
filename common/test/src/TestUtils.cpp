...
TEST_CASE("TestUtilsTest.UVListsEqual", "[TestUtilsTest]") {
  ...
  CHECK_FALSE(UVListsEqual(
    {{0, 0}, {1, 0}, {0, 1}},
    {{10.5, 0}, {11.5, 0}, {10.5, 1}})); // translation by partial texture increments not OK
...
void assertTexture(
  const std::string& expected, const BrushNode* brushNode, const vm::vec3d& v1, const vm::vec3d& v2,
  const vm::vec3d& v3) {
...
void assertTexture(
  const std::string& expected, const BrushNode* brushNode, const std::vector<vm::vec3d>& vertices) {
...
GameAndConfig loadGame(const std::string& gameName) {
  ...
  const auto gamePath =
    IO::Disk::getCurrentWorkingDir() + IO::Path("fixture/test/Model/Game") + IO::Path(gameName);
...
DocumentGameConfig loadMapDocument(
  ...
  document->loadDocument(
    mapFormat, document->worldBounds(), document->game(),
...
int getComponentOfPixel(
  const Assets::Texture& texture, const std::size_t x, const std::size_t y,
...
void checkColor(
  const Assets::Texture& texture, const std::size_t x, const std::size_t y, const int r,
  const int g, const int b, const int a, const ColorMatch match) {
...
TEST_CASE("TestUtilsTest.testUnorderedApproxVecMatcher", "[TestUtilsTest]") {
  ...
  CHECK_THAT(
    (V{{1, 1, 1}}), !UnorderedApproxVecMatches(
