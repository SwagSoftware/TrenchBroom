...
using ExpectedCall = std::variant<
  DoCanAddChild, DoCanRemoveChild, DoParentWillChange, DoParentDidChange, DoAncestorWillChange,
  DoAncestorDidChange>;
...
public:
   ...
   * include a value to return when that function is called, or checks to perform on the function
   * arguments.
   *
...
private: // implement Node interface
  Node* doClone(const vm::bbox3& /* worldBounds */) const override { return new MockNode(); }
...
private: // implement Node interface
  Node* doClone(const vm::bbox3& /* worldBounds */) const override { return new TestNode(); }
...
TEST_CASE("NodeTest.isAncestorOf", "[NodeTest]") {
  ...
  CHECK(
    child1->isAncestorOf(std::vector<Node*>{&root, child1, child2, grandChild1_1, grandChild1_2}));
  CHECK_FALSE(
    child2->isAncestorOf(std::vector<Node*>{&root, child1, child2, grandChild1_1, grandChild1_2}));
  CHECK_FALSE(grandChild1_1->isAncestorOf(
...
TEST_CASE("NodeTest.isDescendantOf", "[NodeTest]") {
  ...
  CHECK_FALSE(
    root.isDescendantOf(std::vector<Node*>{&root, child1, child2, grandChild1_1, grandChild1_2}));
  CHECK(child1->isDescendantOf(
    std::vector<Node*>{&root, child1, child2, grandChild1_1, grandChild1_2}));
  CHECK(child2->isDescendantOf(
    std::vector<Node*>{&root, child1, child2, grandChild1_1, grandChild1_2}));
  CHECK(grandChild1_1->isDescendantOf(
    std::vector<Node*>{&root, child1, child2, grandChild1_1, grandChild1_2}));
  CHECK(grandChild1_1->isDescendantOf(
    std::vector<Node*>{&root, child1, child2, grandChild1_1, grandChild1_2}));
}
...
const auto nodeTestVisitor = kdl::overload(
  ...
  [](BrushNode*) {
    return Visited::Brush;
  },
  [](PatchNode*) {
    return Visited::Patch;
  });
...
TEST_CASE("NodeTest.acceptAndVisitChildren", "[NodeTest]") {
  ...
  CHECK_THAT(
    ...
    Catch::Equals(
      std::vector<Node*>{&world, layer, entityNode1, entityNode2, groupNode, groupEntityNode}));
  CHECK_THAT(
    collectRecursively(*groupNode), Catch::Equals(std::vector<Node*>{groupNode, groupEntityNode}));
...
static auto makeCollectVisitedNodesVisitor(std::vector<Node*>& visited) {
  return kdl::overload(
    ...
    [&](BrushNode* brush) {
      visited.push_back(brush);
    },
    [&](PatchNode* patch) {
      visited.push_back(patch);
    });
}
